...
\begin{itemize}
  \item $(E_\text{e}+\sum E_\gamma)/p_\text{in}$
  \item $E_\text{e}/p_\text{out}$
  \item $\sum E_\gamma/(p_\text{in}-p_\text{out})$
  \item $EarlyBrem$
  \item $LateBrem$
  \item $\log (\sigma_{\eta\eta})$
  \item $\lvert \eta_\text{GSF}-\eta_\text{cluster} \rvert$
  \item $H/(H+E_\text{e})$
  \item $f_\text{brem} = (p_\text{in}-p_\text{out})/p_\text{in}$
  \item $\chi^2_\textrm{GSF}$ and $\chi^2_\textrm{KF}$
  \item \#hits$_\text{KF}$
  \item GSF $\sigma_{p_\text{T}}/p_\text{T}$
  \item GSF $\ln (p_\text{T})$
  \item GSF $\eta$
\end{itemize}
...
analysis technique, optimised to select isolated electrons from W and Z decays. The following variables are combined
into a single discriminator:

\begin{itemize}
  \item $\chi^2_{\text{GSF}}$: $\chi^2$ of the GSF track
  \item $\chi^2_{\text{KF}}$: $\chi^2$ of the KF track
  \item Number of hits of the KF track
  \item $\Delta\eta$: the distance in $\eta$ between the position of the cluster and the extrapolated position of the
  track
  \item $\Delta\phi$: the distance in $\phi$ between the position of the cluster and the extrapolated position of the
  track
  \item $\Delta\eta_{vtx}$: the distance in $\eta$ between the position of the cluster and the position of the track at
  vertex
  \item $\Delta\phi_{vtx}$: the distance in $\phi$ between the position of the cluster and the position of the track at
  vertex
  \item Cluster shape in $\eta$, $\sigma_{i\eta i\eta}$
  \item Cluster shape in $\phi$, $\sigma_{i\phi i\phi}$
  \item $\eta$ width of the super-cluster
  \item $\phi$ width of the super-cluster
  \item $1-\frac{E_{1\times 5}}{E_{5\times 5}}$ where $E_{1\times 5}$ is the energy in the central $1\times 5$ strip of
  the $5\times 5$ electron cluster and $E_{5\times 5}$ its total energy
  \item Ratio of the energy of a cluster of $3\times 3$ and the uncorrected (raw) energy of the super-cluster
  $\left(\frac{E_{3\times 3}}{E_{\text{super-cluster, raw}}}\right)$
  \item Ratio of the energy in the HCAL and ECAL ($H/E$)
  \item Ratio of the supercluster energy and the momentum of the associated track
  $\left(\frac{E_{\text{super-cluster}}}{P_{\text{track}}}\right)$
  \item $\frac{1}{E_{\text{super-cluster}}} - \frac{1}{P_{\text{track}}}$
  \item Ratio of the electron ECAL energy and the momentum of the track calculated in the outermost tracker
  $\left(\frac{E_{\text{ECAL}}}{P_{\text{track}}}\right)$
  \item Ratio of the energy in the preshower detector and the raw super-cluster energy \\
  $\left(\frac{E_{\text{PS}}}{E_{\text{super-cluster, raw}}}\text{, only in the endcaps}\right)$
  \item \pt and $\eta$
\end{itemize}
...
%explain more about isolation, photon conversions and corresponding observables
