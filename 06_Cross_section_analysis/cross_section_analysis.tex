...
The full list of systematic uncertainties for the normalised cross section measurement with respect to the \MET variable
is shown in Table~\ref{tab:combined_MET_systematics}.

\input{06_Cross_section_analysis/tables/xsection_MET_systematics}

% \begin{description}[wide=\parindent]
% \item Luminosity: 4.4\% (global)
% \item Electron efficiency (combined trigger + ID + isolation): 3\% (electron events, flat in \pt and $\eta$) (from
% tag-and-probe)
% \item Muon efficiency (combined trigger + ID + isolation): 1\% (muon events) (from tag-and-probe)
% \item QCD shape uncertainty
% \item Single top cross section: 30\% (global). Though the theoretical uncertainties of the NNLO predictions of the
% single top production cross sections in the three production channels are of the order of 5-10\%, it has been agreed
% within the Top PAG to use a combined uncertainty of 30\% for the single top production cross section in the phase space
% of \ttbar production, i.e.\ events with more than three selected jets.
% \item $Q^2$ scale for W and Z events (shape-changing, from dedicated samples, affecting W and Z samples)
% \item Matching scale for W and Z events (shape-changing, from dedicated samples, affecting W and Z samples)
% \item $Q^2$ scale for \ttbar events (shape-changing, from dedicated samples, affecting \ttbar samples)
% \item Matching scale for \ttbar events (shape-changing, from dedicated samples, affecting \ttbar samples)
% \item Jet energy scale (JES) uncertainty: \pt/$\eta$-dependent uncertainties provided by the JetMET group, taking into
% account uncertainties due to jet flavours and pile-up conditions (shape-changing, different for individual samples and
% categories)
% \item Jet energy resolution (JER) uncertainty: $\pm 10$\% (shape-changing, different for individual samples and
% categories)
% \item Unclustered energy: $10\%$ (shape-changing)
% \item Tau energy: $\pm 3\%$ from the TAU POG (shape-changing)
% \item Electron \& photon energy: $\pm 0.6\%$ in EB and $\pm 1.5\%$ in EE. From
% ECAL DPG /Egamma POG (shape-changing)
% \item Muon momentum: $\pm 0.2\%$ as discussed with the muon POG.
% Negligible (shape-changing)
% \item pile up (shape-changing, affecting all simulated samples): 5\% variations of the inelastic cross section used as
% input in the pile-up reweighting procedure
% \item $b$ tagging (for b-flavoured jets with $\pt < \SI{800}{\GeV}$): \pt-dependent, 1.6-8\% (different scales for
% individual samples and categories)~\cite{CMS-PAS-BTV-11-004}
% \item $b$ tagging (for c-flavoured jets with $\pt < \SI{800}{\GeV}$): twice the uncertainty for b-flavoured jets
% (different scales for individual samples and categories, fully correlated with $b$ tagging uncertainty)
% \item $b$ tagging (for c- and b-flavoured jets with $\pt > \SI{800}{\GeV}$): twice the uncertainty at \SI{800}{\GeV}
% (different scales for individual samples and categories, fully correlated with $b$ tagging uncertainty)
% \item mistagging (or $b$ tagging for light-flavoured jets): O(10\%) (different scales for individual samples and
% categories, uncorrelated with $b$ tag uncertainty for $c$/$b$-flavoured jets)~\cite{CMS-PAS-BTV-11-004}
% \end{description}
...
(Section~\ref{ss:MC_generators}). Results are also compared with theoretical predictions showing the effects due to
variations of modelling parameters, including matching threshold and normalisation scale
(Section~\ref{ss:matching_and_factorisation}). In all figures, the inner error bars represent the combined fitting and
unfolding uncertainties, whereas the outer error bars show the uncertainty due to systematic variations. Naturally, the
systematic errors due to matching threshold and normalisation scale choice are excluded from comparison with variations
due to these parameters, and the hadronisation systematic is excluded from the comparison with different generators.
Overall, the results are consistent with predictions of the Standard Model.
