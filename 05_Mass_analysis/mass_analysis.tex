...
on the mass measurement is quoted as a systematic uncertainty.

\item [Overall Jet Energy Scale.] We vary the jet energy of all jets within the ($p_T$ and $\eta$-dependent) 1-sigma
uncertainty band obtained from independent jet energy scale and resolution studies~\autocite{JEC_7TeV}. The
uncertainties used for data and MC can are shown in Fig.~\ref{fig:jes}.

The uncertainty contains various contributions, for pile-up corrections, absolute energy calibration, the relative scale
between data and MC, and a 1-2\% term covering possible discrepancies between data and the simulation in the modelling
of jet flavour dependence of the jet energy response.

As in the kinematic fit the light-quark jet pair is constrained to the W boson mass, the sensitivity of the top mass
measurement to the light-jet quark scale is reduced. When applying the asymmetric up (or down) 1-sigma uncertainty 
(about 1-2\% in the region of interest) to all jets, an effect of $\pm$ 1.2~GeV of the fitted mass is observed. 

%When only applying this variation to b-quark jets, the shift corresponds to about 2/3 of this value. Applying the
%variation only to all non-b jets, yields 1/3 of the overall shift. We quote the value obtained by applying the
%variations fully correlated to all jets, which is believed to provide proper coverage for the current systematic
%uncertainty due to the Jet Energy Scale.

To obtain even better understanding for future measurements of the top mass, it will be useful to propagate individual
sources contributing to the jet energy scale calibration to the final top mass measurement.

As the uncertainty due to a possible difference between light and b-jet energy scale is already included in the overall
jet energy scale uncertainty, no separate uncertainty is quoted for the b-jet energy scale.


\item [Jet energy resolution.]
Jet asymmetry measurements indicate that the jet energy resolution (JER) in data is about 7-20\% worse compared to the
current Monte Carlo simulation~\autocite{JEC_7TeV, JER_7TeV}. To account for this in the calculation of
our central values, the jet energy resolution in simulation is shifted up by 7-20\%, depending on $\eta$. To account for
the resolution uncertainty, two additional shifts according to $\pm$1 $\sigma$s are evaluated. The largest resulting
shifts in top mass with respect to the default are quoted as systematic uncertainties. Due to lack of time, this study
is not yet available and the result obtained in~\cite{TOP-10-009} is quoted.
%To estimate the effect of the $\pm 10\%$ uncertainty, we perform pseudo-experiments with $t \bar t$ and W+jets and
%compare the fitted mass in the nominal sample with 10\% smearing to the sample without JER rescaling and to jets with a
%sample with JER increased by 20\% with respect to the original simulation. We find that a 10\% change in JER
%corresponds to a 5\% change in top mass resolution, 3\% change in the width of the pull, and no significant change in
%mean fitted mass. We quote the largest of the up and down variation, 0.07 GeV, as a systematic uncertainty.

%\item \textbf{MET scale}
  
%The missing transverse energy is used as input to the constrained fit. We vary the unclustered energy by 10\% to
%evaluate the effect on the mass measurement. For practical reasons, we define everything in the event except the 4
%leading jets as the unclustered energy, which means that our observed bias is probably overestimating the real
%uncertainty. The observed shift in fitted mass is +0.41~GeV when scaling the unclustered energy up by 10\%, and
%-0.39~GeV when scaling the unclustered energy down by 10\%. We quote 0.4~GeV as uncertainty.

\item [Factorization scale.]
...
%  \emph{An additional cross-check we can do would be to look at the top mass as a function of the number of jets in the event.}  
...
%To estimate the effect of differences in the underlying event simulation,
%$t \bar t$ and W+jets samples produced with the underlying event tunes D6T and Z2 are compared.
...
%\emph{Still to be updated for the new analysis with b-tagging; this uncertainty in principle double-counts 
% with the overall JES systematic, so in a future analysis it may disappear or be strongly reduced.}  

\item [Pileup.]
%The Monte Carlo samples used in this measurement were all produced without any pileup events. To estimate the impact of
%pileup on our measurement, the samples without pileup are compared to samples that include the simulation of a pileup
%scenario ($\approx 2.2$ pile-up events on average) roughly corresponding to the data taken at the end of the 2010 LHC
%pp running. Thanks to new Jet Energy Corrections which correct for the average expected additional energy from pile-up
%based on the the number of reconstructed primary vertices on event-by-event basis, the full difference is reduced from
%+1.4~GeV to +0.4~GeV. We apply this difference as a -0.4 GeV correction and quote 25\% of the correction as a
...
%\emph{it could be argued that we should apply a 0.41 +- 0.41 GeV downward correction on the measured top mass for the BG composition}
