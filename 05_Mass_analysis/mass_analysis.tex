...
\subsubsection*{Permutation weights}
For each permutation, the relative weight $w_i$ is given as:
\begin{equation}
w_i = \textrm{exp}(-\frac{1}{2}\chi^2) w_{\rm btag}
\end{equation}

This analysis uses b-tagging information to further separate \ttbar signal and background, and to separate \ttbar signal
with correct and wrong jet permutation assignments.  We use the Combined Secondary Vertex (CSV) tagging algorithm at the
Medium working point (requiring CSV discriminant to be greater than 0.679).
