...
The performance of MC generators is usually optimised for one or more of these steps, therefore they are often used
in combination, e.g.\ \ttbar signal is modelled with \MADGRAPH and \PYTHIA. Standalone generators can also be used for
modelling the theory systematic effects, which is also exploited in this thesis. %(\MCATNLO, \POWHEG + \PYTHIA).
...
the threshold. Similarly, the systematic effect of the choice if factorisation scale $Q^2$ (described in
...
\input{05_Mass_analysis/tables/top_mass_mc_systematics}
...
\GEANTfour (GEometry ANd Tracking \autocite{GEANT4}) package, which is implemented in the CMS software
\CMSSW \autocite{CMSSW}. This package fully recreates the geometry of the detector, including all its subsystems as
well as a detailed map of the magnetic field.
...
The simulated MC events are generated at the centre of mass energy of \SI{7}{\TeV}, using the CTEQ6L Parton Distribution
