...
%How do we extract the minimum? Is the parabolic fit performed for a given assumed top quark mass, or a whole set of MC
% masses? Which plot to show? The standard deviation -> top mass error? 3 lowest points? Logically mention the
...
\num{10000} pseudo-experiments were performed with events picked from Monte Carlo samples representing different sources
...
log-likelihood is formed and the top quark mass is extracted in the same way as in the real measurement described
above.

The mass bias, defined as the difference between the average measured mass and the true (generated) top quark mass, is
shown in Figure~\ref{fig:bias_pull_vs_mass} as a function of the generated top quark mass. Clearly, by using the
ideogram method out of the box, there is a significant mass bias which needs to be corrected for. Additionally, the
width of the pull distribution (standard deviation of a Gaussian fit) as a function of the generated top quark mass
before any calibration is also shown. The pull here is defined as the mass bias divided by the statistical uncertainty
of the measured top quark mass, and the pull width is the standard deviation of a Gaussian fit to the pull distribution.
Essentially, it suggests whether the ideogram method evaluates the statistical uncertainty correctly. It can be seen
that the width of the pull is approximately \SI{12}{\pc} larger than unity, which means that this effect also needs to
be corrected for.

%why is the mass bias linear with the true top mass?
...
As a closure test, the mass bias and pull width as a function of true top quark mass are calculated after applying the
full calibration. The distributions are shown in Figure~\ref{fig:calibrated_bias_pull_vs_mass}. Evidently, the closure
tests confirm that the top quark mass and its statistical uncertainty are unbiased after calibration.
