...
The kinematic fit is performed by minimising the following $\chi^{2}$ function:
\begin{equation}
...
and leptons are considered to be massless. The full description of the fitting algorithm can be found in the appendix of
the \HitFit author's PhD thesis \autocite{Snyder}.

% For each combination, if the fitting algorithm converges and a minimum is found, a $\chi^{2}$ value is calculated for
% that combination. Otherwise the combination is rejected. Some variable resulting from the fit for the combination with
% smaller $\chi^{2}$ are shown in Fig.~\ref{fig:fitplots}.
...
% If an event doesn't have a combination which results in converging fit, the event is rejected. For each event which has
% at least one converging combinations, we further impose a loose cut on the $\chi^{2}$ value, $\chi^{2} < 20$, and
% consider only combinations which pass this cut.  Events which don't have at least one converging permutation that pass
% this $\chi^{2}$ cut are rejected and not used in the mass extraction.

% The requirement of at least a converging combination which pass the loose $\chi^{2}$ cut is considered the final
% selection criteria in addition to the reference lepton+jets selection criteria. Table \ref{tab:chi2_eff} list the number
% of events which pass the reference selection only, and number of events which pass both the reference selection and the
% loose $\chi^{2}$ cut.
